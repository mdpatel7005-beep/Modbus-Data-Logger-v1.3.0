...
import type { ReadingInsert } from "../src/types/domain.js";
...
class HistorianTablePool {
  readonly tables = new Map<string, ColumnMetadata[]>();
  readonly rawTimestamps: string[] = [];
  readonly statements: string[] = [];
  rawInsertGate: Promise<void> | null = null;
  onRawInsert: (() => void) | null = null;
  ...
  private createTable(sql: string): void {
    ...
    for (const match of sql.matchAll(
      /"([a-z][a-z0-9_]*)" NUMERIC\(30, (\d+)\)/g,
    )) {
      columns.push({
        column_name: match[1] as string,
        data_type: "numeric",
    ...
    return {
      query: async <T extends QueryResultRow>(
        ...
        if (sql.includes("FROM information_schema.columns")) {
          ...
          return result(
            (this.tables.get(table) ?? []).map((column) => ({ ...column })),
          ) as unknown as QueryResult<T>;
        }
        if (sql.includes("FROM pg_index")) {
          const table = String(parameters?.[0]);
          return result(
            this.tables.has(table) ? [{ column_name: "timestamp" }] : [],
          ) as unknown as QueryResult<T>;
        }
        ...
        if (drop) {
          const [tableName, columnName] = drop.slice(1) as [string, string];
          this.tables.set(
            tableName,
            (this.tables.get(tableName) ?? []).filter(
        ...
        if (insert) {
          const tableName = insert[1] as string;
...
test("disconnects writes, recreates a deleted table on connect, and resumes saving", async (context) => {
  ...
  assert.deepEqual(
    pool.tables.get("reconnect_meter_raw")?.map((column) => column.column_name),
    ["timestamp", "power"],
  );
  assert.equal(database.getDevice(device.id)?.postgresSchemaDirty, false);

  let releaseRawInsert = () => {};
  pool.rawInsertGate = new Promise<void>((resolve) => {
    releaseRawInsert = resolve;
  });
  let notifyRawInsert = () => {};
  const rawInsertStarted = new Promise<void>((resolve) => {
  ...
  const olderQueuedReading = reading(
    device.id,
    register.id,
...
test("connect leaves saving disabled when orphan columns need confirmation", async (context) => {
  ...
  assert.equal(database.getDevice(device.id)?.postgresSchemaDirty, true);
  assert.equal(
    pool.statements.some((statement) => statement.includes("DROP COLUMN")),
    false,
  );
