...
import {
  buildExactHistorianCreateSql,
  buildLastValueDownsampleSql,
  configuredHistorianColumns,
  historianNumericType,
  historianSaveIsDue,
  HistorianSchemaWarningTracker,
  planHistorianColumnRenames,
  roundHistorianValue,
} from "../src/services/postgres-historian.js";
import type { RegisterDefinition } from "../src/types/domain.js";
...
test("plans exact timestamp-plus-tags historian tables", () => {
  const base: Omit<
    RegisterDefinition,
    "id" | "name" | "historianColumn" | "decimalPlaces" | "enabled"
  > = {
    deviceId: "dev_1",
    address: 0,
    functionCode: 3,
...
test("plans safe historian column renames without merging duplicates", () => {
  ...
  assert.deepEqual(
    planHistorianColumnRenames(
      "meter_raw",
      ["timestamp", "power_reg_123456"],
      pending,
    ),
    pending,
  );
  assert.deepEqual(
    planHistorianColumnRenames("meter_raw", ["timestamp", "power"], pending),
    [],
  );
  assert.throws(
    () =>
      planHistorianColumnRenames(
        "meter_raw",
        ["timestamp", "power_reg_123456", "power"],
        pending,
      ),
    /will not merge values automatically/,
  );
});
...
test("builds one valid last-value bucket expression and preserves nulls", () => {
  ...
  assert.match(
    sql,
    /COALESCE\(EXCLUDED\."voltage_reg1", "meter_15m"\."voltage_reg1"\)/,
  );
});
