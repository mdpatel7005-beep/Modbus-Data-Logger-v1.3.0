...
test("renames historian columns in both tables and clears pending state", async (context) => {
  ...
  const timestamp: ColumnMetadata = {
  ...
  const client = {
    async query<T extends QueryResultRow>(sql: string, parameters?: unknown[]) {
      statements.push(sql);
      if (sql.includes("FROM information_schema.columns")) {
        return result(
          (tables.get(String(parameters?.[0])) ?? []).map((column) => ({
            ...column,
          })) as unknown as T[],
        );
      }
      if (sql.includes("FROM pg_index")) {
        return result([{ column_name: "timestamp" }] as unknown as T[]);
      }
      const rename = sql.match(
        /ALTER TABLE "([^"]+)"\s+RENAME COLUMN "([^"]+)"\s+TO "([^"]+)"/,
      );
      if (rename) {
        ...
        if (!column) throw new Error("rename source was not found");
  ...
  assert.equal(
    statements.filter((sql) => sql.includes("RENAME COLUMN")).length,
    2,
  );
  ...
  assert.deepEqual(
    tables.get("rename_meter_raw")?.map((column) => column.column_name),
    ["timestamp", "power"],
  );
  assert.deepEqual(
    tables.get("rename_meter_1m")?.map((column) => column.column_name),
    ["timestamp", "power"],
  );
});
