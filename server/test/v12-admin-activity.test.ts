...
const temporaryRoot = mkdtempSync(
  path.join(tmpdir(), "modbus-data-logger-v12-http-"),
);
...
const { LoggerDatabase, MAX_ACTIVITY_LOG_ENTRIES } = await import(
  "../src/db/database.js"
);
...
test("authentication fails closed for unknown users and invalidates sessions after account changes", async (context) => {
  ...
  const unknownUserToken = await new SignJWT({
    ...
    .sign(
      new TextEncoder().encode(
        "v12-test-secret-with-more-than-32-characters",
      ),
    );
  ...
  assert.equal(
    (await auth.verify(diagnosticLogin.token))?.role,
    "diagnostic",
  );
...
test("overview, activity redaction, status transitions, and cap use real data", (context) => {
  ...
  assert.equal(
    overview.sampleTrend.reduce((sum, bucket) => sum + bucket.samples, 0),
    1,
  );
  ...
  assert.equal(
    activity.items.filter((item) => item.event === "device.status_changed")
      .length,
    1,
  );
  ...
  assert.equal(
    JSON.stringify(audit?.details).includes("must-not-appear"),
    false,
  );
...
test("admin manages monitoring and diagnostic users with protected invariants", async () => {
  ...
  try {
    ...
    const viewer = viewerResponse.json<{ id: string; enabled: boolean }>();
    ...
    const viewerHeaders = {
      authorization: `Bearer ${viewerLogin.json<{ token: string }>().token}`,
    };
    assert.equal(
      (
        await app.inject({
    ...
    const diagnosticHeaders = {
      authorization: `Bearer ${
        diagnosticLogin.json<{ token: string }>().token
      }`,
    };
    ...
    assert.equal(activity.json<{ pageSize: number }>().pageSize, 1);
    assert.ok(activity.json<{ total: number }>().total >= 1);
    assert.equal(
      (
        await app.inject({
          method: "GET",
    ...
    assert.equal(disableSelf.json<{ error: string }>().error, "self_protection");
    ...
    assert.equal(
      demoteLastAdmin.json<{ error: string }>().error,
      "last_administrator",
    );
    ...
    assert.equal(deleteSelf.json<{ error: string }>().error, "self_protection");
    ...
    assert.equal(
      JSON.stringify(users.json()).includes("password_hash"),
      false,
    );
  } finally {
    await app.close();
