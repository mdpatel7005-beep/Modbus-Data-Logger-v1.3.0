...
test("decodes IEEE-754 floats with common word orders", () => {
  assert.ok(
    Math.abs(decodeRegisters([0x4145, 0x70a4], "float32", "ABCD") - 12.34) <
      0.0001,
  );
  assert.ok(
    Math.abs(decodeRegisters([0x70a4, 0x4145], "float32", "CDAB") - 12.34) <
      0.0001,
  );
});
...
test("rejects an incomplete multi-register value", () => {
  assert.throws(
    () => decodeRegisters([0x4145], "float32"),
...
test("groups nearby registers without exceeding the device block size", () => {
  ...
  assert.deepEqual(
    blocks[0]?.registers.map((item) => item.id),
    ["r1", "r2"],
  );
...
test("plans every tag in 1000+ sparse mixed-area maps within protocol limits", () => {
  ...
  const registers: RegisterDefinition[] = Array.from(
    { length: 1_205 },
    (_, index) => {
      ...
      return {
        ...
        functionCode,
        dataType:
          functionCode === 1 || functionCode === 2 ? "bool" : "float64",
        byteOrder: "ABCD",
        scale: 1,
        offset: 0,
        unit: "",
  ...
  assert.ok(
    blocks.every((block) =>
      block.functionCode === 1 || block.functionCode === 2
        ? block.length <= 2_000
        : block.length <= 125,
    ),
  );
  assert.ok(
    blocks.every((block) =>
      block.registers.every(
        (register) => register.functionCode === block.functionCode,
      ),
    ),
  );
});
