...
const temporaryRoot = mkdtempSync(
  path.join(tmpdir(), "modbus-data-logger-data-servers-"),
);
...
const {
  DatabasePublicationSource,
  DataServerSettingsRepository,
} = await import("../src/services/data-servers.js");
const { buildApplication } = await import("../src/app.js");

function deviceInput(name: string, unitId = 1) {
  return {
    name,
...
test("stores safe disabled defaults and publishes original cached words", async (context) => {
  const directory = mkdtempSync(
    path.join(tmpdir(), "modbus-data-server-repository-"),
  );
  ...
  const published = await new DatabasePublicationSource(
    database,
  ).getPublishedDevices();
  ...
  const offlinePublished = await new DatabasePublicationSource(
    database,
  ).getPublishedDevices();
...
test("data-server API enforces admin writes and diagnostic read-only access", async (context) => {
  ...
  assert.deepEqual(
    deviceResponses.map((response) => response.statusCode),
    [201, 201],
  );
  const deviceIds = deviceResponses.map(
  ...
  const diagnosticHeaders = {
    authorization: `Bearer ${diagnosticLogin.json<{ token: string }>().token}`,
  };
  const viewerHeaders = {
