...
import {
  ...
  PostgresHistorian,
} from "../src/services/postgres-historian.js";
import type {
  PostgresSettingsInput,
  ReadingInsert,
} from "../src/types/domain.js";

const settings: PostgresSettingsInput = {
  enabled: true,
...
function createHistorianDevice(database: LoggerDatabase) {
...
class FakePostgresPool {
  ...
  onQuery: (() => void) | null = null;
  endCount = 0;
  readonly rawTimestamps = new Set<string>();
  downsampleWrites = 0;
...
test("pauses new historian work and drains an in-flight write", async () => {
  ...
  try {
    ...
    let releaseQuery = () => {};
    pool.queryGate = new Promise<void>((resolve) => {
...
test("clears transient maintenance and replay status on system reload", async () => {
  ...
  try {
    ...
    const mutable = historian as unknown as {
      lastMaintenanceAtMemory: string | null;
      lastReplayAtMemory: string | null;
      lastReplayCountMemory: number;
    };
    ...
    assert.equal(
      historian.getPublicSettings().lastMaintenanceAt,
      "2026-07-25T01:00:00.000Z",
    );
    assert.equal(
      historian.getPublicSettings().lastReplayAt,
...
test("persists, compacts, caps, and cascades the PostgreSQL outbox", () => {
  ...
  try {
    ...
    const first = reading(
      device.id,
      register.id,
      "2026-07-25T00:00:00.100Z",
      1,
    );
    const latestInBucket = reading(
      device.id,
      register.id,
      "2026-07-25T00:00:00.900Z",
      2,
    );
    database.enqueuePostgresOutbox(
      device.id,
      first.timestamp,
    ...
    database.enqueuePostgresOutbox(
      device.id,
      "2026-07-25T00:00:01.100Z",
      Date.parse("2026-07-25T00:00:01.000Z"),
      [reading(device.id, register.id, "2026-07-25T00:00:01.100Z", 3)],
      2,
    );
    const overflow = database.enqueuePostgresOutbox(
      device.id,
      "2026-07-25T00:00:02.100Z",
      Date.parse("2026-07-25T00:00:02.000Z"),
      [reading(device.id, register.id, "2026-07-25T00:00:02.100Z", 4)],
      2,
    );
...
test("queues unavailable writes and replays them before the next live sample", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      [...pool.rawTimestamps],
      [cached.timestamp, live.timestamp],
    );
    assert.equal(database.getPostgresOutboxStats().queuedRows, 0);
...
test("queues schema-dirty samples and pauses replay until schema synchronization", async () => {
  ...
  try {
    ...
    const cachedNextBucket = reading(
      device.id,
      register.id,
      "2026-07-25T00:00:01.100Z",
      13.456,
    ...
    assert.equal(pool.rawTimestamps.size, 0);
    assert.equal(
      warnings.filter((message) => message.includes("durable offline cache"))
        .length,
      1,
    );
    ...
    assert.equal(
      database.markDevicePostgresSchemaSynced(
        device.id,
        dirtyDevice.postgresSchemaRevision,
      ),
      true,
    );
    ...
    assert.deepEqual(
      [...pool.rawTimestamps],
      [cached.timestamp, cachedNextBucket.timestamp],
    );
    assert.equal(database.getPostgresOutboxStats().queuedRows, 0);
...
test("recognizes availability errors without hiding schema failures", () => {
  ...
  assert.equal(
    isPostgresAvailabilityError(new Error("Query read timeout")),
    true,
  );
  assert.equal(
    isPostgresAvailabilityError(
