...
import { AttributeIds, DataType, OPCUAClient, StatusCodes, Variant, } from "node-opcua";
import { ensureOpcUaCertificateIdentity, OpcUaPublisher, } from "../src/services/opc-ua-publisher.js";
...
function publishedDevice(value = 12.5) {
    return {
        id: "device-a",
        name: "Boiler PLC",
        tags: [
...
function config(port, pkiDirectory) {
    return {
        enabled: true,
        host: "127.0.0.1",
...
test("publishes read-only OPC UA values with quality and source timestamps", { timeout: 30_000 }, async () => {
    const port = await freePort();
    const pkiDirectory = await mkdtemp(join(tmpdir(), "modbus-data-logger-opcua-"));
    ...
    const publisher = new OpcUaPublisher(source, config(port, pkiDirectory));
    ...
    try {
        ...
        const temperatureNode = "ns=1;s=Devices/device-a/temperature";
        ...
        assert.equal(value.sourceTimestamp?.toISOString(), "2026-07-27T04:30:00.000Z");
        const quality = await activeSession.readVariableValue(`${temperatureNode}/Quality`);
        ...
        assert.equal(unavailable.statusCode, StatusCodes.BadNoCommunication);
        ...
        assert.equal(unavailableAfterRefreshFailure.statusCode, StatusCodes.BadNoCommunication);
        ...
        const writeStatus = await activeSession.writeSingleNode(temperatureNode, new Variant({ dataType: DataType.Double, value: 999 }));
...
test("startup failure is retained in status and does not escape to the collector", async () => {
    ...
    const publisher = new OpcUaPublisher(source, {
    ...
    assert.match(publisher.getStatus().lastError ?? "", /cannot be inside a public folder/);
});
test("refresh rebuilds the OPC UA address space when the selected tag topology changes", async () => {
    ...
    const runtimeFactory = async (_config, runtimeDevices) => {
        starts += 1;
        topologySizes.push(runtimeDevices.reduce((count, device) => count + device.tags.length, 0));
        return {
    ...
    const publisher = new OpcUaPublisher(source, config(4_840, "/var/lib/modbus-data-logger/opcua"), runtimeFactory);
    await publisher.start();
...
test("non-anonymous OPC UA requires and uses an authentication provider", async () => {
    ...
    let runtimeCreated = false;
    const noProvider = new OpcUaPublisher(source, secureConfig, async () => {
        runtimeCreated = true;
    ...
    assert.match(noProvider.getStatus().lastError ?? "", /no authentication provider/);
    let callbacks = null;
    const securePublisher = new OpcUaPublisher(source, secureConfig, async (_config, _devices, runtimeCallbacks) => {
        ...
        validateCredentials: async (username, password) => username === "monitor" && password === "simple",
    });
    await securePublisher.start();
    assert.equal(securePublisher.getStatus().state, "running");
    const authenticate = callbacks?.authenticateUser;
    assert(authenticate);
...
test("wildcard OPC UA binding requires a separate advertised host", async () => {
    ...
    let runtimeCreated = false;
    const missingAdvertisedHost = new OpcUaPublisher(source, {
        ...config(4_840, "/var/lib/modbus-data-logger/opcua"),
        host: "0.0.0.0",
    ...
    assert.match(missingAdvertisedHost.getStatus().lastError ?? "", /advertised host is required/);
    let receivedAdvertisedHost;
    const configured = new OpcUaPublisher(source, {
        ...config(4_840, "/var/lib/modbus-data-logger/opcua"),
        host: "0.0.0.0",
        ...
        return {
            getEndpointUrl: () => "opc.tcp://logger.example.internal:4840/test",
            getConnectedClientCount: () => 0,
            getCurrentChannelCount: () => 0,
            getCurrentSessionCount: () => 0,
            getCurrentSubscriptionCount: () => 0,
    ...
    assert.equal(receivedAdvertisedHost, "logger.example.internal");
    await configured.stop();
});
test("advertised-host change safely rotates the persisted self-signed certificate", { timeout: 30_000 }, async () => {
    const pkiDirectory = await mkdtemp(join(tmpdir(), "modbus-data-logger-opcua-rotation-"));
    const certificateFile = join(pkiDirectory, "own", "certs", "certificate.pem");
    const privateKeyFile = join(pkiDirectory, "own", "private", "private_key.pem");
    const source = {
        getPublishedDevices: async () => [publishedDevice()],
    };
    let initialPublisher = null;
    let changedPublisher = null;
    try {
        ...
        const initialCertificate = new X509Certificate(await readFile(certificateFile));
        assert.equal(initialCertificate.checkHost("logger-one.example", {
            subject: "never",
        }), "logger-one.example");
        ...
        const changedCertificate = new X509Certificate(await readFile(certificateFile));
        assert.equal(changedCertificate.checkHost("logger-two.example", {
            subject: "never",
        }), "logger-two.example");
        assert.notEqual(changedCertificate.fingerprint256, initialCertificate.fingerprint256);
        assert.deepEqual(await readFile(privateKeyFile), initialPrivateKey);
        assert.equal((await readdir(join(pkiDirectory, "own", "certs"))).some((name) => name.includes(".rotation-backup-")), false);
    }
    finally {
...
test("failed certificate rotation restores the previous certificate", async () => {
    const directory = await mkdtemp(join(tmpdir(), "modbus-data-logger-opcua-rollback-"));
    ...
    let invalidations = 0;
    try {
        await assert.rejects(ensureOpcUaCertificateIdentity({
            certificateFile,
        ...
        }, "logger-new.example"), /previous certificate was restored/);
        assert.deepEqual(await readFile(certificateFile), original);
        assert.equal(invalidations, 2);
        assert.equal((await readdir(directory)).some((name) => name.includes(".rotation-backup-")), false);
    }
    finally {
...
test("a CA or GDS certificate is never replaced automatically", async () => {
    const directory = await mkdtemp(join(tmpdir(), "modbus-data-logger-opcua-ca-"));
    const certificateFile = join(directory, "certificate.pem");
    ...
    try {
        await assert.rejects(ensureOpcUaCertificateIdentity({
            ...
            invalidateCachedCertificates: () => { },
            endpoints: [],
        }, "logger-new.example"), /CA\/GDS certificate.*provision a replacement certificate/);
        assert.deepEqual(await readFile(certificateFile), managedCertificate);
        assert.equal((await readdir(directory)).some((name) => name.includes(".rotation-backup-")), false);
    }
    finally {
        await rm(directory, { recursive: true, force: true });
...
//# sourceMappingURL=opc-ua-publisher.test.js.map