...
const applicationRoot = mkdtempSync(path.join(tmpdir(), "modbus-large-tag-http-"));
...
function tagPayload(index) {
    ...
    return {
        ...
        functionCode,
        dataType: functionCode === 1 || functionCode === 2
            ? "bool"
            : functionCode === 3
                ? "uint16"
                : "float32",
...
test("bulk imports 1000+ mixed tags atomically and returns every live tag", async (context) => {
    context.after(() => rmSync(applicationRoot, { force: true, recursive: true }));
    ...
    const deviceId = createdDevice.json().id;
    ...
    assert.equal(new Set(liveBody.items.map((item) => item.registerId)).size, 1_205);
    assert.ok(liveBody.items.every((item) => item.value === null &&
        item.quality === "bad" &&
        item.hasReading === 0));
    ...
    assert.equal(caseOnlyNameConflict.json().error, "tag_name_conflict");
    ...
    assert.equal(tagsAfterRollback.json().items.length, 1_205);
    const fillToLimit = await app.inject({
        method: "POST",
        url: `/api/v1/devices/${deviceId}/registers/import`,
        headers: authorization,
        payload: {
            items: Array.from({ length: 295 }, (_, offset) => tagPayload(1_205 + offset)),
        },
    });
    assert.equal(fillToLimit.statusCode, 201, fillToLimit.body);
    assert.equal(fillToLimit.json().totalTags, 1_500);
    ...
    assert.equal(overLimit.json().error, "device_tag_limit");
...
//# sourceMappingURL=large-tag-http.test.js.map