...
const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), "modbus-data-logger-http-"));
...
test("supports login, password, device, tag, and device-live workflows", async () => {
    ...
    try {
        ...
        authorization = {
            authorization: `Bearer ${newLogin.json().token}`,
        };
        ...
        assert.equal(postgresSettings.json()
        ...
        assert.equal(savedPostgresSettings.json().host, "db.example.internal");
        assert.equal(savedPostgresSettings.json()
            .historianTimezone, "Asia/Kolkata");
        ...
        assert.equal(maintenance.json().skipped, true);
        ...
        assert.equal(invalidClassificationDevice.json().error, "invalid_device_classification");
        ...
        assert.match(classifiedExport.body, /^timestamp,deviceName,categoryName,groupName,tagName,address,value,unit,quality/);
        ...
        assert.match(invalidGroupAssignment.json().message, /group was not found/);
        ...
        assert.equal(conflictingHistorianColumn.json().error, "historian_column_conflict");
        ...
        assert.deepEqual(Object.keys(unconfiguredSchemaSync.json()).sort(), [
            "addedColumns",
            "changedColumns",
            "droppedColumns",
            "message",
        ...
        assert.equal(unconfiguredSchemaSync.json().ok, false);
        ...
        assert.equal(tagEdit.json().historianColumn, "line_voltage");
        assert.equal(tagEdit.json().decimalPlaces, 4);
        ...
        assert.equal(deviceEdit.json().saveIntervalMs, 1000);
        assert.equal(deviceEdit.json()
            .postgresRawRetentionDays, 7);
        const editedDevice = deviceEdit.json();
        ...
        assert.equal(tagsResponse.json().items.length, 1);
        ...
        assert.deepEqual({
            ...liveBody.items[0],
            timestampIsValid: Number.isFinite(Date.parse(liveBody.items[0]?.timestamp ?? "")),
            timestamp: undefined,
        }, {
            id: 0,
            registerId: tag.id,
        ...
        assert.equal(disconnect.json().connected, false);
        assert.equal(disconnect.json().device
            .postgresEnabled, false);
        ...
        assert.equal(repeatDisconnect.json().connected, false);
        ...
        assert.equal(connectWithoutGlobalConfiguration.json()
            .connected, false);
        assert.equal(connectWithoutGlobalConfiguration.json().device.postgresEnabled, false);
        assert.equal(connectWithoutGlobalConfiguration.json().device.postgresSchemaDirty, true);
        const auditInspection = new SqliteDatabase(process.env.DATABASE_PATH, { readonly: true });
        ...
        assert.deepEqual(connectionAuditActions.map((row) => row.action), [
            "postgres.device_disconnect",
            "postgres.device_disconnect",
            "postgres.device_connect_attention",
        ]);
...
//# sourceMappingURL=http-workflow.test.js.map