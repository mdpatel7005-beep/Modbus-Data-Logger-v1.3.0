...
const temporaryRoot = mkdtempSync(path.join(os.tmpdir(), "modbus-system-admin-"));
...
const encryptionKey = "system-admin-test-encryption-key-more-than-32-characters";
...
test("secures updates, VPN, encrypted backup restore, and factory reset", async (context) => {
    ...
    const pathTestDatabase = new LoggerDatabase(path.join(temporaryRoot, "path-test.db"));
    assert.throws(() => new SystemAdministrationService(pathTestDatabase, {
    ...
    assert.equal(health.json().version, "1.1.0");
    ...
    const operatorDelete = await app.inject({
        method: "DELETE",
        url: `/api/v1/users/${operatorCreate.json().id}`,
        headers: authorization,
    });
    ...
    assert.match(stagedUpdate.json().stagedSha256, /^[a-f0-9]{64}$/);
    const stagedUpdatePath = path.join(systemDataPath, "updates", "staged-update.pkg");
    ...
    assert.match(rejectedUpdateStatus.json().update.lastError ?? "", /did not accept/i);
    writeFileSync(updateHelperPath, '#!/bin/sh\n[ "$1" = "apply" ] || exit 2\n[ -f "$2" ] || exit 3\nexit 0\n', { mode: 0o700 });
    chmodSync(updateHelperPath, 0o700);
    ...
    const validProfile = Buffer.from("client\ndev tun\nremote vpn.example.test 1194\n<ca>\nTEST-CA\n</ca>\n");
    ...
    const deviceId = deviceResponse.json().id;
    ...
    const registerId = registerResponse.json().id;
    ...
    const envelope = JSON.parse(decryptSecret(backup.slice(backupPrefix.length)));
    ...
    assert.match(String(envelope.data.postgresSettings[0]?.password_encrypted), /^v1\./);
    ...
    authorization = {
        authorization: `Bearer ${refreshedLogin.json().token}`,
    };
    ...
    const stageBeforeRestore = async () => app.inject({
        method: "POST",
        url: "/api/v1/settings/system/update/stage",
        headers: {
            ...authorization,
    ...
    staleVersionDatabase
        .prepare("UPDATE system_update_state SET staged_version = '1.1.0' WHERE id = 1")
        .run();
    ...
    const activeVpnBackup = `${backupPrefix}${encryptSecret(JSON.stringify(activeVpnEnvelope))}`;
    ...
    const restoredAt = restoreResponse.json().restoredAt;
    assert.equal(existsSync(stagedUpdatePath), false);
    assert.equal(restoreResponse.json().collectorReloaded, true);
    const statusAfterRestore = await app.inject({
        method: "GET",
        url: "/api/v1/settings/system",
        headers: authorization,
    });
    assert.equal(statusAfterRestore.json().openVpn.enabled, false);
    assert.equal(statusAfterRestore.json().update.stagedVersion, null);
    assert.match(statusAfterRestore.json().openVpn.lastError ?? "", /restored/i);
    const devicesAfterRestore = await app.inject({
        method: "GET",
        url: "/api/v1/devices",
        headers: authorization,
    });
    const portableRestoredDevice = devicesAfterRestore.json().items[0];
    ...
    assert.deepEqual(normalizedRuntime
        .prepare(`SELECT
    ...
    assert.deepEqual(restoredClassifications
        .json()
        .categories.map((item) => item.name), ["Energy"]);
    ...
    const invalidBackup = `${backupPrefix}${encryptSecret(JSON.stringify(invalidEnvelope))}`;
    ...
    assert.deepEqual(configurationAfterRejectedRestore
        .json()
        .categories.map((item) => item.name), ["Energy"]);
    ...
    writeFileSync(openVpnHelperPath, `#!/bin/sh
...
`, { mode: 0o700 });
    ...
    direct
        .prepare(`INSERT INTO postgres_outbox (
          device_id, sample_timestamp, save_bucket_ms, readings_json,
          created_at
        ) VALUES (?, ?, ?, ?, ?)`)
        .run(deviceId, "2026-07-25T12:00:00.000Z", 1_000, "[]", "2026-07-25T12:00:00.000Z");
    direct.close();
    ...
    assert.deepEqual(inspection.prepare("SELECT action FROM audit_log").all(), [{ action: "system.factory_reset" }]);
    inspection.close();
...
//# sourceMappingURL=system-admin.test.js.map