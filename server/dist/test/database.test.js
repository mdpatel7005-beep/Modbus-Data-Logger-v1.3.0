...
import { HistorianColumnNameConflictError, LoggerDatabase, } from "../src/db/database.js";
test("stores devices, registers, and readings transactionally", (context) => {
    ...
    assert.throws(() => database.createDeviceCategory("energy METERS"), /UNIQUE constraint failed/);
    ...
    assert.throws(() => database.deleteDeviceCategory(category.id), /FOREIGN KEY constraint failed/);
    ...
    assert.equal(database.findHistorianTableOwner("modbus_raw", "modbus_1m", "another_device"), undefined);
    assert.match(register.historianColumn, /^[a-z][a-z0-9_]{0,62}$/);
    const firstSync = "2026-07-25T09:00:00.000Z";
    assert.equal(database.markDevicePostgresSchemaSynced(device.id, 1, firstSync), true);
    assert.equal(database.getDevice(device.id)?.postgresSchemaSyncedAt, firstSync);
    ...
    assert.equal(database.markDevicePostgresSchemaSynced(device.id, 1, "2026-07-25T09:30:00.000Z"), false);
    assert.equal(database.getDevice(device.id)?.postgresSchemaDirty, true);
    database.markDevicePostgresSchemaSynced(device.id, 2, firstSync);
    assert.equal(database.findHistorianTableOwner("modbus_raw", "modbus_1m", "another_device")?.id, device.id);
    ...
    const latestReadings = database.getLatestReadings(10, device.id);
    ...
    const readings = database.queryReadings({ limit: 10 });
    ...
    assert.equal(database.queryReadings({ categoryId: "cat_missing", limit: 10 }).length, 0);
    ...
    database.markDevicePostgresSchemaSynced(device.id, 4, "2026-07-25T10:00:00.000Z");
    assert.equal(database.deleteRegister(register.id), true);
    assert.equal(database.getDevice(device.id)?.postgresSchemaSyncedAt, "2026-07-25T10:00:00.000Z");
    assert.equal(database.getDevice(device.id)?.postgresSchemaDirty, true);
...
test("uses readable historian columns and reserves pending renames", (context) => {
    ...
    assert.throws(() => database.createRegister({
        ...baseRegister,
    ...
    database.acknowledgeHistorianColumnRenames(device.id, database.listPendingHistorianColumnRenames(device.id));
    assert.deepEqual(database.listPendingHistorianColumnRenames(device.id), []);
});
test("migrates decimal precision and schema-sync state onto existing data", (context) => {
    ...
    original.connection.exec("ALTER TABLE registers DROP COLUMN postgres_previous_column_name");
    original.connection.exec("ALTER TABLE registers DROP COLUMN decimal_places");
    original.connection.exec("ALTER TABLE devices DROP COLUMN postgres_schema_synced_at");
    original.connection.exec("ALTER TABLE devices DROP COLUMN postgres_schema_dirty");
    original.connection.exec("ALTER TABLE devices DROP COLUMN postgres_schema_revision");
    ...
    assert.equal(migrated.getRegister(register.id)?.historianColumn, "existing_tag_123abc");
