...
const temporaryRoot = mkdtempSync(path.join(tmpdir(), "modbus-system-admin-data-servers-"));
...
const { DataServerSettingsRepository } = await import("../src/services/data-servers.js");
...
test("backs up, safely restores, and resets data-server configuration", async (context) => {
    ...
    assert.equal(envelope.data.dataServerSettings[0]?.opcua_advertised_host, "logger.test.internal");
    ...
    const legacyBackup = `${backupPrefix}${encryptSecret(JSON.stringify(legacyEnvelope))}`;
    ...
    assert.equal(database.connection
        .prepare("SELECT COUNT(*) AS count FROM data_server_device_exports")
        .get().count, 0);
    repository.save({
        modbus: {
            enabled: true,
    ...
    assert.deepEqual(database.connection
        ...
        .all(), [{ event: "system.factory_reset" }]);
});
//# sourceMappingURL=system-admin-data-servers.test.js.map