...
test("aborts an active Modbus transport before administration continues", async () => {
    ...
    let rejectConnect = () => { };
    const blockedConnect = new Promise((_resolve, reject) => {
    ...
    const poller = new PollingService(activeDatabase, logger, historian, true, () => client);
    await poller.start();
...
test("polls 1,205 sparse mixed-function tags without dropping readings and flushes live data incrementally", { timeout: 5_000 }, async () => {
    const functionCodes = [1, 2, 3, 4];
    const registers = Array.from({ length: 1_205 }, (_, index) => {
        const functionCode = functionCodes[index % functionCodes.length];
        ...
        return {
            ...
            address,
            functionCode,
            dataType: functionCode === 1 || functionCode === 2 ? "bool" : "uint16",
            byteOrder: "ABCD",
            scale: 1,
            offset: 0,
            unit: "",
    ...
    const plannedBlocks = groupRegisters(registers, largeDevice.readBlockSize);
    ...
    const poller = new PollingService(largeDatabase, logger, largeHistorian, true, () => client);
    await poller.start();
    try {
        await finalBlockStarted.promise;
        const incrementallyFlushed = insertedBatches.flat();
        assert.ok(incrementallyFlushed.length >= 250, "the live-reading threshold should flush before the final block");
        assert.ok(incrementallyFlushed.length < registers.length, "the first live flush must happen before the full cycle is available");
        ...
        for (const block of requestedBlocks) {
            const protocolLimit = block.functionCode === 1 || block.functionCode === 2 ? 2_000 : 125;
            assert.ok(block.length > 0);
            assert.ok(block.length <= Math.min(largeDevice.readBlockSize, protocolLimit), `FC${block.functionCode} block length ${block.length} exceeded its configured/protocol limit`);
        }
        ...
        assert.deepEqual(liveReadings.map((reading) => reading.registerId).sort(), expectedIds);
        assert.ok(liveReadings.every((reading) => reading.quality === "good"));
        assert.equal(historianWrites.length, 1);
        const historianReadings = historianWrites[0];
        assert.equal(historianReadings.length, registers.length);
        assert.deepEqual(historianReadings.map((reading) => reading.registerId).sort(), expectedIds);
    }
    finally {
        releaseFinalBlock.resolve();
        await poller.stopAndDrain();
...
test("learns a failed block split and preserves seven good tags around one bad address", { timeout: 5_000 }, async () => {
    ...
    const registers = addresses.map((address) => ({
    ...
    const splitDatabase = {
        ...
        updateDeviceHealth(_deviceId, status) {
            healthStatuses.push(status);
            if (healthStatuses.length === 2)
                twoCyclesFinished.resolve();
        },
    };
    ...
    const poller = new PollingService(splitDatabase, logger, splitHistorian, true, () => {
        const requestedBlocks = [];
        ...
        return {
            ...
            async readBlock(block) {
                requestedBlocks.push(block);
                if (block.registers.some((register) => register.address === 199)) {
    ...
    try {
        ...
        const firstCycle = requestedBlocksByCycle[0];
        const secondCycle = requestedBlocksByCycle[1];
        assert.equal(firstCycle.length, 3);
        assert.deepEqual(firstCycle[0]?.registers.map((register) => register.address), addresses);
        assert.deepEqual(firstCycle[1]?.registers.map((register) => register.address), addresses.slice(0, -1));
        assert.deepEqual(firstCycle[2]?.registers.map((register) => register.address), [199]);
        assert.equal(secondCycle.length, 2, "the next poll should skip the known-failing parent block");
        assert.deepEqual(secondCycle[0]?.registers.map((register) => register.address), addresses.slice(0, -1));
        assert.deepEqual(secondCycle[1]?.registers.map((register) => register.address), [199]);
        assert.equal(insertedReadings.length, addresses.length * 2);
        const secondCycleReadings = insertedReadings.slice(addresses.length);
        const goodReadings = secondCycleReadings.filter((reading) => reading.quality === "good");
        const badReadings = secondCycleReadings.filter((reading) => reading.quality === "bad");
        assert.equal(goodReadings.length, 7);
        assert.deepEqual(goodReadings
            .map((reading) => Number(reading.registerId.split("_").at(-1)))
            .sort((left, right) => left - right), addresses.slice(0, -1));
        ...
        assert.ok(historianWrites.every((readings) => readings.length === addresses.length));
...
test("hard-limits adaptive fallback requests when every nested block fails", { timeout: 5_000 }, async () => {
    const failedDevice = {
        ...device,
        id: "dev_failed_fallback",
        name: "Failed fallback device",
    };
    const registers = Array.from({ length: 32 }, (_, index) => ({
    ...
    const poller = new PollingService(failedDatabase, logger, historian, true, () => client);
    await poller.start();
    try {
        await cycleFinished.promise;
        assert.ok(requestedBlocks.length <= 1 + 8, `expected one parent plus at most eight fallback requests, received ${requestedBlocks.length}`);
        assert.equal(insertedReadings.length, registers.length);
        assert.ok(insertedReadings.every((reading) => reading.quality === "bad"));
    }
    finally {
        await poller.stopAndDrain();
    }
});
//# sourceMappingURL=poller-lifecycle.test.js.map