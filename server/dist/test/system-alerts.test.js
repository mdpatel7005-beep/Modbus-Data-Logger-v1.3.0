...
const applicationRoot = mkdtempSync(path.join(tmpdir(), "modbus-system-alert-http-"));
...
const { PostgresHistorian } = await import("../src/services/postgres-historian.js");
...
function postgresSettingsInput(overrides = {}) {
    return {
        enabled: true,
...
function createDevice(database) {
...
test("deduplicates durable incidents, debounces outages, and orders WhatsApp recovery", async (context) => {
    ...
    const requests = [];
    const service = new SystemAlertService(database, logger, async (url, init) => {
        requests.push({
            url: String(url),
            authorization: new Headers(init?.headers).get("authorization") ?? "",
            body: JSON.parse(String(init?.body)),
        });
        return new Response(JSON.stringify({ messages: [{ id: `wamid.${requests.length}` }] }), { status: 200, headers: { "content-type": "application/json" } });
    });
    ...
    const storedToken = database.connection
        .prepare("SELECT access_token_encrypted FROM whatsapp_alert_settings WHERE id = 1")
        .get();
    assert.notEqual(storedToken.access_token_encrypted, "meta-secret-token");
    assert.equal(decryptSecret(storedToken.access_token_encrypted), "meta-secret-token");
    const common = {
        type: "device_offline",
    ...
    assert.equal(database.observeSystemAlert({
        ...common,
    ...
    assert.equal(database.listSystemAlerts({ activeOnly: true, limit: 250 }).length, 1);
    ...
    assert.equal(database.connection
        .prepare("SELECT COUNT(*) AS count FROM system_alert_deliveries WHERE alert_id = ?")
        .get(first.id).count, 0, "an unaccepted outage is canceled without a stale recovery");
    database.observeSystemAlert({
        ...common,
    ...
    assert.equal(requests[0]?.url, "https://graph.facebook.com/v23.0/123456789012345/messages");
    assert.equal(requests[0]?.authorization, "Bearer meta-secret-token");
    const parameters = (requests[0]?.body).template.components[0]?.parameters;
    assert.deepEqual(parameters?.slice(0, 4).map((parameter) => parameter.text), ["OFFLINE", "Device offline", "Boiler PLC", "Connection refused"]);
    database.observeSystemAlert({
        ...common,
        offline: false,
        detail: "Device communication recovered",
    ...
    const recoveryParameters = (requests[1]?.body).template.components[0]?.parameters;
    assert.deepEqual(recoveryParameters?.slice(0, 4).map((parameter) => parameter.text), [
        "RECOVERED",
        "Device offline",
        "Boiler PLC",
    ...
    assert.equal(database.listSystemAlerts({ activeOnly: false, limit: 250 }).length, 2, "device deletion does not erase outage history");
});
test("retries transient Meta failures, makes permanent failures dead, and cancels removed recipients", async (context) => {
    ...
    const service = new SystemAlertService(database, logger, async () => new Response("{}", {
    ...
    await service.flushDeliveries();
    assert.deepEqual(database.connection
    ...
    assert.equal(database.connection
        .prepare("SELECT COUNT(*) AS count FROM system_alert_deliveries WHERE recipient = ?")
        .get("919876543211").count, 0);
    database.connection
        .prepare("UPDATE system_alert_deliveries SET next_attempt_at = ? WHERE status = 'failed'")
        .run(new Date(0).toISOString());
    responseStatus = 400;
    await service.flushDeliveries();
    assert.deepEqual(database.connection
        .prepare("SELECT status, attempts FROM system_alert_deliveries")
        .all(), [{ status: "dead", attempts: 2 }]);
    service.saveSettings({
        ...settingsInput(),
        enabled: false,
        recipients: ["919876543210"],
    ...
    assert.equal(database.connection
        .prepare("SELECT COUNT(*) AS count FROM system_alert_deliveries WHERE status IN ('pending', 'failed')")
        .get().count, 0);
});
test("tests an unsaved WhatsApp draft without persisting its token or endpoint fields", async (context) => {
    ...
    const service = new SystemAlertService(database, logger, async (url, init) => {
        ...
        authorization = new Headers(init?.headers).get("authorization") ?? "";
        return new Response(JSON.stringify({ messages: [{ id: "wamid.draft" }] }), { status: 200 });
    });
    context.after(() => service.close());
    service.saveSettings({
    ...
    assert.equal(requestedUrl, "https://graph.facebook.com/v23.0/222222222222222/messages");
    assert.equal(authorization, "Bearer draft-token");
    assert.equal(database.getWhatsAppAlertSettings()?.phoneNumberId, "111111111111111");
    assert.equal(decryptSecret(database.getWhatsAppAlertSettings()?.accessTokenEncrypted ?? ""), "saved-token");
});
test("PostgreSQL monitoring opens only for availability failures and resolves on a successful probe", async (context) => {
    ...
    assert.equal(database.getDevice(device.id)?.lastSeenAt, "2026-07-27T02:00:00.000Z", "a failed poll preserves the last successful observation");
    ...
    const historian = new PostgresHistorian(database, logger, () => pool, alerts);
    ...
    assert.equal(database.listSystemAlerts({ activeOnly: true, limit: 250 }).length, 1, "an authentication/configuration error is not recovery or a new outage");
    failureCode = null;
    assert.deepEqual(await historian.checkAvailability(), {
        monitored: true,
        online: true,
    });
    assert.equal(database.listSystemAlerts({ activeOnly: true, limit: 250 }).length, 0);
...
test("PostgreSQL settings disable and target replacement immediately resolve stale outages and cancel unsent alerts", async (context) => {
    const directory = mkdtempSync(path.join(tmpdir(), "modbus-pg-settings-alert-"));
    context.after(() => rmSync(directory, { force: true, recursive: true }));
    const database = new LoggerDatabase(path.join(directory, "logger.db"));
    context.after(() => database.close());
    ...
    const unsentCount = () => database.connection
...
test("a late failure from the replaced PostgreSQL pool cannot reopen a resolved outage", async (context) => {
    const directory = mkdtempSync(path.join(tmpdir(), "modbus-pg-generation-alert-"));
    context.after(() => rmSync(directory, { force: true, recursive: true }));
    const database = new LoggerDatabase(path.join(directory, "logger.db"));
    context.after(() => database.close());
    const alerts = new SystemAlertService(database, logger, async () => {
    ...
    let rejectOldProbe = (error) => {
        void error;
    };
    const oldProbe = new Promise((_resolve, reject) => {
    ...
    const historian = new PostgresHistorian(database, logger, () => {
        poolsCreated += 1;
        return (poolsCreated === 1 ? oldPool : replacementPool);
    }, alerts);
    context.after(() => historian.close());
    ...
    rejectOldProbe(Object.assign(new Error("old target refused the connection"), {
        code: "ECONNREFUSED",
    }));
...
test("configuration backups include encrypted WhatsApp settings, accept V1 backups, and reset operational alert data", async (context) => {
    ...
    const envelope = JSON.parse(decryptSecret(encoded));
    assert.equal(envelope.data.whatsappAlertSettings?.length, 1);
    assert.equal(envelope.data.whatsappAlertSettings?.[0]?.enabled, 1);
    assert.notEqual(envelope.data.whatsappAlertSettings?.[0]?.access_token_encrypted, "meta-secret-token");
    ...
    const legacyBackup = `modbus-data-logger-backup.v1.${encryptSecret(JSON.stringify(envelope))}`;
    ...
    assert.equal(Boolean(database.getWhatsAppAlertSettings()?.accessTokenEncrypted), true);
    assert.equal(database.listSystemAlerts({ activeOnly: false, limit: 250 }).length, 0);
...
test("system alert and WhatsApp APIs enforce RBAC without leaking access tokens", async (context) => {
    ...
    async function createAndLoginRole(role) {
        ...
        return {
            authorization: `Bearer ${roleLogin.json().token}`,
        };
    }
    ...
    assert.equal(save.json().accessTokenConfigured, true);
    ...
    assert.equal(listAsViewer.json().items.length, 1);
...
//# sourceMappingURL=system-alerts.test.js.map