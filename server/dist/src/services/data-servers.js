...
import { OpcUaPublisher, } from "./opc-ua-publisher.js";
...
function safeRawWords(value) {
    if (!value)
        return [];
    try {
        const parsed = JSON.parse(value);
        return Array.isArray(parsed)
            ? parsed.filter((word) => Number.isInteger(word) && word >= 0 && word <= 0xffff)
            : [];
    }
    catch {
        return [];
...
function suggestedMappings(deviceIds, stored) {
    const used = new Set([...stored.values()]
        .filter((item) => item.modbus_enabled === 1)
        .map((item) => item.modbus_unit_id));
    let nextUnitId = 1;
    const nextAvailable = () => {
        while (used.has(nextUnitId) && nextUnitId <= 247)
            nextUnitId += 1;
    ...
    return deviceIds.map((device) => {
        ...
        const unitId = preferred >= 1 && preferred <= 247 && !used.has(preferred)
            ? preferred
            : nextAvailable();
        used.add(unitId);
...
export class DataServerSettingsRepository {
    ...
    migrate() {
        this.database.connection.transaction(() => {
            this.database.connection
                .prepare(`CREATE TABLE IF NOT EXISTS data_server_settings (
            ...
            const settingsColumns = this.database.connection
                ...
                .all();
            if (!settingsColumns.some((column) => column.name === "opcua_advertised_host")) {
                this.database.connection
                    .prepare(`ALTER TABLE data_server_settings
             ADD COLUMN opcua_advertised_host TEXT NOT NULL
             DEFAULT '127.0.0.1'`)
                    .run();
            }
            this.database.connection
                .prepare(`CREATE TABLE IF NOT EXISTS data_server_device_exports (
    ...
    getInput() {
        ...
        const exports = this.database.connection
            .prepare(`SELECT device_id, modbus_enabled, modbus_unit_id, opcua_enabled
         FROM data_server_device_exports`)
            .all();
        const exportByDevice = new Map(exports.map((item) => [item.device_id, item]));
    ...
    save(input) {
        const existingDevices = new Set(this.database.listDevices().map((device) => device.id));
        for (const item of [
        ...
        this.database.connection.transaction(() => {
            this.database.connection
                .prepare(`UPDATE data_server_settings SET
             modbus_enabled = ?,
             modbus_bind_address = ?,
             modbus_port = ?,
             ...
             updated_at = ?
           WHERE id = 1`)
                .run(Number(input.modbus.enabled), input.modbus.bindAddress, input.modbus.port, input.modbus.refreshIntervalMs, Number(input.opcUa.enabled), input.opcUa.bindAddress, input.opcUa.advertisedHost, input.opcUa.port, input.opcUa.endpointPath, Number(input.opcUa.allowAnonymous), input.opcUa.refreshIntervalMs, now);
            this.database.connection
                .prepare("DELETE FROM data_server_device_exports")
                .run();
            const insert = this.database.connection.prepare(`INSERT INTO data_server_device_exports (
           device_id, modbus_enabled, modbus_unit_id, opcua_enabled
         ) VALUES (?, ?, ?, ?)`);
            for (const [deviceId, item] of combined) {
                insert.run(deviceId, Number(item.modbusEnabled), item.unitId, Number(item.opcUaEnabled));
            }
        })();
    }
    disableAll() {
        this.database.connection
            .prepare(`UPDATE data_server_settings
...
export class DatabasePublicationSource {
    ...
    async getPublishedDevices() {
        ...
        for (const row of rows) {
            ...
            const reading = row.device_enabled !== 1 ||
...
export class DataServerManager {
    ...
    opcUa;
    monitorTimer = null;
    lastStatusSignatures = new Map();
    lastAppliedInput;
    fullyStopped = false;
    constructor(database, logger, pkiDirectory, opcUaAuthenticationProvider) {
        this.database = database;
        this.logger = logger;
        ...
        this.opcUa = new OpcUaPublisher(this.source, this.opcUaConfig(input), undefined, opcUaAuthenticationProvider);
    }
    async start() {
    ...
    async reload() {
        const input = this.repository.getInput();
        const opcUaChanged = this.fullyStopped ||
    ...
    async reloadOpcUa() {
        const input = this.repository.getInput();
        if (!input.opcUa.enabled || input.opcUa.allowAnonymous)
            return;
        await this.opcUa.reload(this.opcUaConfig(input));
        this.lastAppliedInput = structuredClone(input);
        this.recordStatusChanges();
...
//# sourceMappingURL=data-servers.js.map