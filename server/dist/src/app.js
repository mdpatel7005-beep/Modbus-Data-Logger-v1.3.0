...
import { DeviceRegisterLimitError, HistorianColumnNameConflictError, LoggerDatabase, RegisterNameConflictError, UserAdministrationError, } from "./db/database.js";
...
import { HistorianAdministrationPausedError, PostgresHistorian, } from "./services/postgres-historian.js";
import { SystemAdministrationService } from "./services/system-admin.js";
import { SystemAlertService, WhatsAppAlertConfigurationError, } from "./services/system-alerts.js";
export async function buildApplication() {
    ...
    const postgresHistorian = new PostgresHistorian(database, app.log, undefined, systemAlerts);
    const poller = new PollingService(database, app.log, postgresHistorian);
    poller.setSystemAlertService(systemAlerts);
    const dataServers = new DataServerManager(database, app.log, path.join(env.systemAdminDataDir, "opcua-pki"), {
        validateCredentials: (username, password) => auth.validateReadOnlyCredentials(username, password),
    });
    const systemAdministration = new SystemAdministrationService(database);
    let retentionTimer = null;
    let postgresMonitorTimer = null;
    ...
    app.addHook("onReady", async () => {
        ...
        const runRetention = async () => {
            ...
            if (deleted > 0) {
                app.log.info({ deleted, retentionDays: env.retentionDays }, "expired readings removed");
            }
            try {
    ...
    app.addHook("onClose", async () => {
        ...
        if (postgresMonitorTimer)
            clearInterval(postgresMonitorTimer);
