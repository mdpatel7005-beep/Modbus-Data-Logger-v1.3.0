...
export function registerWidth(dataType: RegisterDataType): number {
  switch (dataType) {
    case "uint32":
    case "int32":
    case "float32":
...
function registersToBytes(registers: number[]): Buffer {
...
function reorder(buffer: Buffer, byteOrder: ByteOrder): Buffer {
  if (buffer.length === 2) {
  ...
  const words = Array.from({ length: buffer.length / 2 }, (_, index) =>
    buffer.subarray(index * 2, index * 2 + 2),
  );
  ...
  return Buffer.concat(
    orderedWords.map((word) =>
      swapBytes ? Buffer.from([word[1] ?? 0, word[0] ?? 0]) : word,
    ),
  );
}
...
export function decodeRegisters(
  ...
  if (registers.length < registerWidth(dataType)) {
    throw new Error(
      `Expected ${registerWidth(dataType)} registers for ${dataType}, received ${registers.length}`,
    );
  }
  ...
  const buffer = reorder(
    registersToBytes(registers.slice(0, registerWidth(dataType))),
    byteOrder,
  );
