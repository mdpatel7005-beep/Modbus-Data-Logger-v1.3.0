  ...
  .object({
    ...
    customerCode: z
      ...
      .refine(
        ...
        "Customer code may use letters, numbers, dot, underscore, slash, and hyphen",
      ),
    ...
    contactPhone: z
      ...
      .max(50)
      .refine(
        (value) => value === "" || /^\+?[0-9 ()-]{5,50}$/.test(value),
...
const bindAddressSchema = z
  ...
  .refine(
    ...
    "Use a local IP address, localhost, or 0.0.0.0 for all interfaces",
  );
...
const advertisedHostSchema = z
  ...
  .refine(
    ...
    "Use the hostname or IP address that OPC UA clients will connect to",
  );
...
export const dataServerSettingsSchema = z
    ...
    opcUa: z
      .object({
        ...
        endpointPath: z
          ...
          .regex(
            /^\/[A-Za-z0-9._~/-]*$/,
            "Use an OPC UA endpoint path beginning with /",
          )
          .refine(
            (value) => !value.includes("//") && !value.includes(".."),
            "Endpoint path cannot contain // or ..",
          ),
  ...
  .superRefine((value, context) => {
    const mappingDevices = new Set<string>();
    const enabledUnitIds = new Set<number>();
    for (const [index, mapping] of value.modbus.mappings.entries()) {
      ...
      if (mapping.enabled) enabledUnitIds.add(mapping.unitId);
    }
  ...
  .superRefine((value, context) => {
    if (value.enabled && (!value.host || !value.database || !value.username)) {
      context.addIssue({
        ...
        path: ["host"],
        message:
          "Host, database, and username are required when PostgreSQL is enabled",
      });
    }
...
export const registerSchema = z
  ...
  .superRefine((value, context) => {
    if (
      (value.functionCode === 1 || value.functionCode === 2) &&
      value.dataType !== "bool"
    ) {
      context.addIssue({
        code: "custom",
...
export const alarmGroupRuleSchema = z
  ...
  .superRefine((value, context) => {
    if (
      (value.condition === "above" || value.condition === "outside") &&
    ...
    if (
      (value.condition === "below" || value.condition === "outside") &&
...
const whatsAppRecipientSchema = z
  ...
  .regex(
    /^\+?[1-9]\d{7,14}$/,
    "Use an international WhatsApp number, for example 919876543210",
  )
