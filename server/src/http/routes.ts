...
import type { CreateDeviceInput, LoggerDatabase } from "../db/database.js";
import type { Principal, UpdateAlarmRuleInput } from "../types/domain.js";
...
import {
  HistorianSchemaConflictError,
  type HistorianSchemaSyncResult,
  type PostgresHistorian,
} from "../services/postgres-historian.js";
import {
  alarmCategoryRuleSchema,
  alarmGroupMemberSchema,
  alarmGroupRuleSchema,
  alarmGroupSchema,
  alarmRuleSchema,
  alarmsQuerySchema,
  activityQuerySchema,
  ...
  dataServerSettingsSchema,
  deviceClassificationSchema,
  deviceSchema,
  historianSchemaSyncSchema,
  idParamsSchema,
  loginSchema,
  postgresSettingsSchema,
  readingsQuerySchema,
  resetUserPasswordSchema,
  registerImportSchema,
  registerSchema,
  ruleIdParamsSchema,
  systemAlertsQuerySchema,
  updateUserSchema,
  whatsAppAlertSettingsSchema,
} from "./schemas.js";
...
export async function registerRoutes(
  ...
  function parseDeviceInput(body: unknown): CreateDeviceInput {
    ...
    return deviceSchema.parse({
      ...
      postgresDownsampleIntervalSec:
        postgresDefaults.defaultDownsampleIntervalSec,
      ...
      postgresMaintenanceIntervalHours:
        postgresDefaults.maintenanceIntervalHours,
      ...provided,
    }) as CreateDeviceInput;
  }

  function classificationAssignmentError(
  ...
  function requireRole(...roles: Principal["role"][]) {
  ...
  app.addHook("preHandler", async (request, reply) => {
    ...
    await reply.code(503).send({
      ...
      message:
        "Configuration changes are temporarily paused for system administration",
    });
  });
  ...
  app.get(
    ...
    async () => ({ items: database.listUsers() }),
  );
  ...
  app.patch(
    ...
    async (request, reply) => {
      ...
      const user = database.updateUser(
        id,
        input,
        request.principal?.id ?? "",
      );
  ...
  app.delete(
    ...
    async (request, reply) => {
      const { id } = idParamsSchema.parse(request.params);
      const deleted = database.deleteUser(
        id,
        request.principal?.id ?? "",
      );
      if (!deleted) {
        ...
        ...items.map((item) =>
          headers
            .map((header) =>
              csvEscape(
                header === "details"
                  ? JSON.stringify(item.details).slice(
                      0,
                      ACTIVITY_EXPORT_DETAILS_MAX_CHARACTERS,
                    )
  ...
  app.get(
    "/settings/postgres",
    { preHandler: requireRole("administrator") },
    async () => postgresHistorian.getPublicSettings(),
  );

  app.get(
    "/settings/data-servers",
    { preHandler: requireRole("administrator", "diagnostic") },
    async () => dataServers.getSettings(),
  );
  ...
  app.put(
    ...
    async (request, reply) => {
      ...
      const existingDevices = new Set(
        database.listDevices().map((device) => device.id),
      );
      const missing = [
      ...
      database.appendAudit({
        ...
        details: {
          ...
          modbusDeviceCount: input.modbus.mappings.filter(
          ...
          opcUaDeviceCount: input.opcUa.publications.filter(
  ...
  app.post(
    "/settings/device-classifications/categories",
    { preHandler: requireRole("administrator") },
    async (request, reply) => {
  ...
  app.post(
    "/settings/device-classifications/groups",
    { preHandler: requireRole("administrator") },
    async (request, reply) => {
      const { name } = deviceClassificationSchema.parse(request.body);
  ...
  app.delete(
    "/settings/device-classifications/categories/:id",
    { preHandler: requireRole("administrator") },
    async (request, reply) => {
      const { id } = idParamsSchema.parse(request.params);
  ...
  app.delete(
    "/settings/device-classifications/groups/:id",
    { preHandler: requireRole("administrator") },
    async (request, reply) => {
      const { id } = idParamsSchema.parse(request.params);
  ...
  app.put(
    ...
    async (request, reply) => {
      ...
      if (input.enabled) {
        database.recordPostgresConnectionTest(
          true,
          "Connection tested successfully when settings were applied",
        );
      }
      database.appendAudit({
        actorId: request.principal?.id,
  ...
  app.post(
    "/devices",
    { preHandler: requireRole("administrator", "operator") },
    async (request, reply) => {
    ...
    async (request, reply) => {
      ...
      if (input.postgresEnabled && !postgresHistorian.configured) {
        return reply.code(400).send({
          ...
          message:
            "Configure and enable Remote PostgreSQL before enabling it on a device",
        });
      }
      if (!previousDevice.postgresEnabled && input.postgresEnabled) {
        return reply.code(409).send({
          ...
          message:
            "Use Connect and verify to enable PostgreSQL saving so both historian tables are checked first",
        });
      }
      const tableOwner = database.findHistorianTableOwner(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        database.appendAudit({
          actorId: request.principal?.id,
          action:
            error instanceof HistorianSchemaConflictError
        ...
        return reply.code(502).send({
          connected: false,
          message:
            "Remote PostgreSQL verification failed; data saving remains disconnected",
          device,
        });
      }
    },
  ...
  app.post(
    "/devices/:id/historian-schema/sync",
    { preHandler: requireRole("administrator") },
    async (request, reply) => {
      const { id } = idParamsSchema.parse(request.params);
      const { dropRemoved, expectedOrphanedColumns } =
        historianSchemaSyncSchema.parse(request.body ?? {});
      const emptyResult = (message: string): HistorianSchemaSyncResult => ({
        ok: false,
        message,
        addedColumns: [],
        changedColumns: [],
      ...
      try {
        const result = await postgresHistorian.syncDeviceSchema(
          id,
          dropRemoved,
  ...
  app.delete(
    "/registers/:id/alarm-rules/:ruleId",
    { preHandler: requireRole("administrator", "operator") },
    async (request, reply) => {
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      return reply
        ...
        .header(
          "content-disposition",
          `attachment; filename="modbus-readings-${new Date().toISOString().slice(0, 10)}.csv"`,
        )
        .send(csv);
    },
  );
  ...
  app.delete(
    "/alarm-groups/:id/members/:registerId",
    { preHandler: requireRole("administrator", "operator") },
    async (request, reply) => {
      const params = request.params as { id: string; registerId: string };
  ...
  // Get category with matching registers (for device/tag selection)
  app.get(
    ...
    async (request, reply) => {
      ...
      // Get matching registers for this category
      const matchingRegisters = database.getCategoriesWithMatchingRegisters(categoryId);
  ...
  app.put(
    ...
    async (request, reply) => {
      ...
      const validRegisterIds = new Set<string>();
  ...
  app.post(
    ...
    async (request, reply) => {
      const { id } = idParamsSchema.parse(request.params);
      const acknowledged = database.acknowledgeAlarm(
        id,
        request.principal?.id ?? "",
      );
      if (!acknowledged) {
  ...
  app.post(
    "/alerts/system/:id/acknowledge",
    { preHandler: requireRole("administrator", "operator") },
    async (request, reply) => {
      const { id } = idParamsSchema.parse(request.params);
      const acknowledged = database.acknowledgeSystemAlert(
        id,
        request.principal?.id ?? "",
      );
      if (!acknowledged) {
  ...
  app.get(
    "/settings/alerts/whatsapp",
    { preHandler: requireRole("administrator") },
    async () => systemAlerts.getPublicSettings(),
  );
