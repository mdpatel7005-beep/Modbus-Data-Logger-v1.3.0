...
import {
  SystemAdministrationError,
  type SystemAdministrationService,
...
export async function registerSystemRoutes(
  ...
  let administrationTail: Promise<void> = Promise.resolve();
  ...
  async function serializeAdministration<T>(
    ...
    administrationTail = previous.then(
      () => current,
      () => current,
    );
  ...
  async function resumeCollector(): Promise<{
    ...
    const collectorReloaded =
      historianReloaded && pollingRestarted && dataServersReloaded;
    return { collectorReloaded, restartRequired: !collectorReloaded };
  }

  async function withCollectorQuiesced<T>(
    operation: () => Promise<T> | T,
  ): Promise<{
    result: T;
    collectorReloaded: boolean;
    ...
    try {
      ...
      postgresHistorian.resume();
      await poller.resumeAndStart().catch((resumeError: unknown) => {
        app.log.error(
          {
            error:
    ...
    return { result: result as T, ...reload };
  }

  app.get("/settings/system", { onRequest: requireAdministrator }, async () =>
    systemAdministration.getStatus(),
  );
  ...
  app.get(
    ...
    async (request, reply) => {
      try {
        const backup = await serializeAdministration(() =>
          systemAdministration.createConfigurationBackup(auditContext(request)),
        );
        ...
        return reply
          ...
          .header(
            "content-disposition",
            `attachment; filename="modbus-data-logger-configuration-${date}.backup"`,
          )
          .send(backup);
      } catch (error) {
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      try {
        const outcome = await serializeAdministration(() =>
          withCollectorQuiesced(() =>
            systemAdministration.restoreConfigurationBackup(
              input.backup,
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      try {
        const outcome = await serializeAdministration(() =>
          withCollectorQuiesced(() =>
            systemAdministration.factoryReset(auditContext(request)),
          ),
        );
        return reply.code(200).send({
          ok: true,
    ...
    async (request, reply) => {
      ...
      if (!version || !filename || !Buffer.isBuffer(request.body)) {
        return reply.code(400).send({
          ...
          message:
            "A binary body, x-update-version, and x-file-name are required",
        });
      }
      const contents = request.body;
      try {
        const result = await serializeAdministration(() =>
          systemAdministration.stageUpdate(
            contents,
            version,
            filename,
            auditContext(request),
  ...
  app.post(
    ...
    async (request, reply) => {
      try {
        const result = await serializeAdministration(() =>
          systemAdministration.applyUpdate(auditContext(request)),
        );
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      try {
        const result = await serializeAdministration(() =>
          systemAdministration.saveOpenVpnProfile(
            contents,
            filename,
            auditContext(request),
          ),
  ...
  app.put(
    "/settings/system/openvpn",
    { onRequest: requireAdministrator },
    async (request, reply) => {
      const input = openVpnStateSchema.parse(request.body);
      try {
        const result = await serializeAdministration(() =>
          systemAdministration.setOpenVpnEnabled(
            input.enabled,
            auditContext(request),
          ),
        );
