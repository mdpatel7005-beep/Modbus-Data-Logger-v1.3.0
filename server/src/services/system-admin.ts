...
const optionalPostgresIdentifierSchema = postgresIdentifierSchema.or(
  z.literal(""),
);
...
const deviceRowSchema = z
  .object({
    ...
    baud_rate: z
      ...
      .int()
      .refine((value) =>
        [1200, 2400, 4800, 9600, 19_200, 38_400, 57_600, 115_200].includes(
          value,
        ),
      )
      .nullable(),
  ...
  .superRefine((row, context) => {
    ...
    if (row.save_interval_ms < row.poll_interval_ms) {
      context.addIssue({
        code: "custom",
        message:
          "The database save interval cannot be shorter than the polling interval",
      });
    }
    if (
      row.postgres_enabled === 1 &&
      ...
      context.addIssue({
        code: "custom",
        message:
          "The downsample interval cannot be shorter than the database save interval",
      });
    }
  });
...
const alarmRuleRowSchema = z
  ...
  .superRefine((row, context) => {
    if (
      (row.condition === "above" && row.threshold_high === null) ||
  ...
  .superRefine((row, context) => {
    if (
      ...
      context.addIssue({
        code: "custom",
        message:
          "Enabled PostgreSQL settings require host, database, and username",
      });
    }
...
const whatsappAlertSettingsRowSchema = z
  ...
  .superRefine((row, context) => {
    ...
    const validRecipients =
      ...
      recipients.every(
        (recipient) =>
          typeof recipient === "string" && /^[1-9]\d{7,14}$/.test(recipient),
      );
    if (!validRecipients) {
      context.addIssue({
        code: "custom",
...
const dataServerBindAddressSchema = z
  ...
  .refine(
    ...
    "Invalid data-server bind address",
  );
...
const dataServerAdvertisedHostSchema = z
  ...
  .refine(
    ...
    "Invalid OPC UA advertised hostname",
  );
...
const dataServerSettingsRowSchema = z
  .object({
    ...
    opcua_advertised_host: dataServerAdvertisedHostSchema.default(
      "127.0.0.1",
    ),
...
type BackupEnvelope = z.infer<typeof backupEnvelopeSchema>;
type UpdateStateRow = z.infer<typeof updateStateRowSchema>;
type OpenVpnStateRow = z.infer<typeof openVpnStateRowSchema>;
type DataServerSettingsRow = z.infer<typeof dataServerSettingsRowSchema>;

function defaultDataServerSettings(updatedAt: string): DataServerSettingsRow {
  return {
    id: 1,
    modbus_enabled: 0,
...
interface ManagedFileReplacement {
  target: string;
...
function parseSemver(value: string): ParsedSemver | null {
  ...
  const prerelease = prereleaseIdentifiers.map((identifier) =>
    /^(0|[1-9]\d*)$/.test(identifier) ? BigInt(identifier) : identifier,
  );
  return {
...
function safeUploadFilename(value: string, extension: RegExp): string {
  ...
  if (
    ...
    throw new SystemAdministrationError(
      400,
      "invalid_file_name",
...
function normalizeOpenVpnDirective(value: string): string {
  return value.toLowerCase().replace(/^--/, "");
}

function parseOpenVpnDirective(line: string): string {
  const [rawDirective] = line.split(/\s+/, 1);
  if (!rawDirective || !/^(?:--)?[A-Za-z][A-Za-z0-9-]*$/.test(rawDirective)) {
    throw new SystemAdministrationError(
      400,
      "unsafe_openvpn_profile",
      "Quoted, escaped, or malformed OpenVPN directives are not allowed",
    );
  }
  const directive = normalizeOpenVpnDirective(rawDirective);
  if (
    !allowedOpenVpnClientDirectives.has(directive) &&
    !selfContainedOpenVpnDirectives.has(directive)
  ) {
    throw new SystemAdministrationError(
      400,
      "unsafe_openvpn_profile",
      `OpenVPN directive "${directive}" is not supported by the client-profile allowlist`,
    );
  }
  return directive;
}
...
function assertRegularFile(filePath: string): void {
  if (!lstatSync(filePath).isFile()) {
    throw new SystemAdministrationError(
      ...
      "A managed system file is not a regular file",
    );
  }
}
...
function canonicalPathForCreation(target: string): string {
...
function writePrivateFile(filePath: string, contents: Buffer): void {
...
function ensurePrivateDirectory(directory: string): void {
  ...
  if (!metadata.isDirectory() || metadata.isSymbolicLink()) {
    throw new Error(
      "System administration storage must use real private directories",
    );
  }
...
export function validateOpenVpnProfile(contents: Buffer): string {
  if (contents.length === 0 || contents.length > VPN_MAX_BYTES) {
    throw new SystemAdministrationError(
      400,
      "invalid_openvpn_profile",
      "The OpenVPN profile must be between 1 byte and 1 MB",
    );
  }
  ...
  try {
    ...
    throw new SystemAdministrationError(
      400,
      "invalid_openvpn_profile",
      "The OpenVPN profile must contain valid UTF-8 text",
    );
  }
  if (profile.includes("\0")) {
    throw new SystemAdministrationError(
      400,
      "invalid_openvpn_profile",
      "The OpenVPN profile contains invalid control data",
    );
  }
  ...
  let activeInlineBlock: string | null = null;
  ...
  for (const sourceLine of lines) {
      ...
      if (line.toLowerCase() === `</${activeInlineBlock}>`) {
        if (!activeInlineHasContent) {
          throw new SystemAdministrationError(
            400,
            "invalid_openvpn_profile",
            `The inline ${activeInlineBlock} block is empty`,
          );
        }
        if (inlineBlocks.has(activeInlineBlock)) {
          throw new SystemAdministrationError(
            400,
            "invalid_openvpn_profile",
            `The inline ${activeInlineBlock} block is duplicated`,
          );
        }
    ...
    if (directive === "remote") remoteConfigured = true;
  ...
  if (activeInlineBlock) {
    throw new SystemAdministrationError(
      400,
      "invalid_openvpn_profile",
      `The inline ${activeInlineBlock} block is not closed`,
    );
  }
  if (!clientMode) {
    throw new SystemAdministrationError(
      400,
      "invalid_openvpn_profile",
      'The OpenVPN profile must enable client mode with "client" or "tls-client"',
    );
  }
  if (!remoteConfigured) {
    throw new SystemAdministrationError(
      400,
      "invalid_openvpn_profile",
      'The OpenVPN client profile must contain at least one "remote" directive',
    );
  }
  for (const { directive, firstArgument } of inlineReferences) {
    const requiredBlock = selfContainedOpenVpnDirectives.get(directive);
    if (!requiredBlock) continue;
    const explicitlyInline =
      firstArgument === "[inline]" || firstArgument === "inline";
    if (
      !inlineBlocks.has(requiredBlock) ||
      (firstArgument !== undefined && !explicitlyInline)
    ) {
      throw new SystemAdministrationError(
        400,
        "external_openvpn_reference",
        `OpenVPN directive "${directive}" must use embedded inline material`,
      );
    }
  }
...
export class SystemAdministrationService {
  ...
  private readonly systemUpdateHelper: string | undefined;
  private readonly openVpnHelper: string | undefined;

  constructor(
    private readonly database: LoggerDatabase,
    options: SystemAdministrationOptions = {},
  ) {
    this.appVersion = options.appVersion ?? env.appVersion;
    const requestedDataDirectory = path.resolve(
      options.dataDirectory ?? env.systemAdminDataDir,
    );
    ...
    if (
      unsafeDirectories.has(requestedDataDirectory) ||
      requestedDataDirectory.split(path.sep).includes("public")
    ) {
      throw new Error(
        "System administration data must use a dedicated non-public subdirectory",
      );
    }
    if (
      existsSync(requestedDataDirectory) &&
      (!lstatSync(requestedDataDirectory).isDirectory() ||
        lstatSync(requestedDataDirectory).isSymbolicLink())
    ) {
      throw new Error(
        "System administration data must use a dedicated real directory",
      );
    }
    const canonicalDataDirectory = canonicalPathForCreation(
    ...
    const canonicalDatabaseDirectory = realpathSync(
      path.dirname(env.databasePath),
    );
    if (
      [
      ...
      throw new Error(
        "System administration data resolves into an unsafe shared directory",
      );
    }
    ...
    this.updateFilePath = path.join(
      this.dataDirectory,
      "updates",
      "staged-update.pkg",
    );
    this.openVpnProfilePath = path.join(
      this.dataDirectory,
      "openvpn",
      "profile.ovpn",
    );
  ...
  private initialize(): void {
    ...
    this.database.connection.transaction(() => {
      ...
      this.database.connection
        .prepare(
          `CREATE TABLE IF NOT EXISTS openvpn_state (
            id INTEGER PRIMARY KEY CHECK (id = 1),
            profile_name TEXT,
      ...
      this.database.connection
        .prepare(
          `INSERT OR IGNORE INTO openvpn_state (
  ...
  private getUpdateState(): UpdateStateRow {
    return updateStateRowSchema.parse(
      this.database.connection
  ...
  private getOpenVpnState(): OpenVpnStateRow {
    return openVpnStateRowSchema.parse(
      this.database.connection
  ...
  createConfigurationBackup(audit: SystemAuditContext): string {
    ...
    const openVpn = this.getOpenVpnState();
    const profile = this.readManagedFile(
      this.openVpnProfilePath,
      VPN_MAX_BYTES,
      false,
    );
    const normalizedOpenVpn: OpenVpnStateRow = profile
      ? openVpn
      : {
          ...openVpn,
          profile_name: null,
    ...
    const envelope: BackupEnvelope = {
      ...
      data: {
        ...
        dataServerDeviceExports: this.selectAllIfExists(
          "data_server_device_exports",
        ),
    ...
    if (Buffer.byteLength(serialized, "utf8") > BACKUP_MAX_JSON_BYTES) {
      throw new SystemAdministrationError(
        413,
        "configuration_backup_too_large",
        "The configuration is too large for a restorable backup",
      );
    }
    ...
    if (Buffer.byteLength(encrypted, "utf8") > BACKUP_MAX_TEXT_LENGTH) {
      throw new SystemAdministrationError(
        413,
        "configuration_backup_too_large",
        "The encrypted configuration exceeds the restore limit",
      );
    }
  ...
  async restoreConfigurationBackup(
  ...
  ): Promise<{ restoredAt: string; reloadRequired: true }> {
    ...
    const profileContents = envelope.data.openVpnProfileBase64
      ...
      : null;
    if (profileContents) validateOpenVpnProfile(profileContents);
    const restoredAt = new Date().toISOString();
    const restoredCustomerProfile = envelope.data.customerProfile?.map(
      (profile) => ({
        ...profile,
    ...
    const restoredPostgresSettings = envelope.data.postgresSettings.map(
      (settings) => ({
        ...settings,
    ...
    const restoredWhatsappAlertSettings =
      envelope.data.whatsappAlertSettings.map((settings) => ({
        ...settings,
        enabled: 0,
        last_test_at: null,
        last_test_ok: null,
    ...
    const restoredDataServerSettings =
      envelope.data.dataServerSettings.length > 0
        ? envelope.data.dataServerSettings.map((settings) => ({
            ...settings,
            modbus_enabled: 0 as const,
            opcua_enabled: 0 as const,
            updated_at: restoredAt,
          }))
    ...
    this.replaceManagedFiles(
      ...
      () => {
        ...
        this.insertRows(
          ...
          envelope.data.deviceCategories,
        );
        this.insertRows(
          "device_groups",
          ["id", "name", "created_at", "updated_at"],
          envelope.data.deviceGroups,
        );
        this.insertRows(
          "devices",
          [
        ...
        this.database.appendAudit({
          ...
          details: {
            ...
            dataServerMappings:
              envelope.data.dataServerDeviceExports.length,
          },
          sourceIp: audit.sourceIp,
        });
      },
  ...
  async factoryReset(audit: SystemAuditContext): Promise<{
    resetAt: string;
    reloadRequired: true;
  }> {
    if (this.getOpenVpnState().enabled === 1) {
  ...
  stageUpdate(
    ...
    if (contents.length === 0 || contents.length > UPDATE_MAX_BYTES) {
      throw new SystemAdministrationError(
        400,
        "invalid_update",
        "The update package must be between 1 byte and 100 MB",
      );
    }
    ...
    if (
      ...
      throw new SystemAdministrationError(
        ...
        "The update version must be valid semver and newer than this application",
      );
    }
    ...
    const gzipMagic =
      contents.length >= 2 && contents[0] === 0x1f && contents[1] === 0x8b;
    if (
      (lowerName.endsWith(".zip") && !zipMagic) ||
      (lowerName.endsWith(".tar.gz") && !gzipMagic)
    ) {
      throw new SystemAdministrationError(
        400,
        "invalid_update_archive",
        "The update package magic does not match its archive extension",
      );
    }
    ...
    this.replaceManagedFiles(
      [{ target: this.updateFilePath, contents }],
      () => {
        this.upsertUpdateState({
          id: 1,
  ...
  async applyUpdate(audit: SystemAuditContext): Promise<{
    accepted: true;
    version: string;
  }> {
    ...
    if (
      ...
      throw new SystemAdministrationError(
        409,
        "update_not_staged",
        "No complete update package is staged",
      );
    }
    ...
    if (
      ...
      this.recordUpdateError(
        "The staged update is not newer than the running application",
      );
      throw new SystemAdministrationError(
        409,
    ...
    const contents = this.readManagedFile(
      this.updateFilePath,
      UPDATE_MAX_BYTES,
      true,
    );
    if (!contents || sha256(contents) !== state.staged_sha256) {
      this.recordUpdateError(
        "The staged update package failed integrity validation",
      );
      throw new SystemAdministrationError(
        409,
    ...
    if (!isExecutable(this.systemUpdateHelper) || !this.systemUpdateHelper) {
      throw new SystemAdministrationError(
        ...
        "The system update helper is not configured or executable",
      );
    }
    ...
    try {
      ...
      this.recordUpdateError(
        "The system update helper did not accept the package",
      );
      throw new SystemAdministrationError(
        502,
  ...
  saveOpenVpnProfile(
    ...
    if (previous.enabled === 1) {
      throw new SystemAdministrationError(
        409,
        "openvpn_profile_active",
        "Disconnect OpenVPN before replacing its profile",
      );
    }
    this.replaceManagedFiles(
      [{ target: this.openVpnProfilePath, contents }],
      () => {
        this.upsertOpenVpnState({
          id: 1,
  ...
  async setOpenVpnEnabled(
    enabled: boolean,
    audit: SystemAuditContext,
  ): Promise<{ enabled: boolean; changedAt: string }> {
    ...
    if (
      ...
      throw new SystemAdministrationError(
        409,
        "openvpn_profile_required",
        "Upload a self-contained OpenVPN profile before connecting",
      );
    }
    if (!isExecutable(this.openVpnHelper) || !this.openVpnHelper) {
      throw new SystemAdministrationError(
        503,
        "openvpn_helper_unavailable",
        "The OpenVPN helper is not configured or executable",
      );
    }
    ...
    try {
      await this.runHelper(
        this.openVpnHelper,
        enabled ? ["connect", this.openVpnProfilePath] : ["disconnect"],
      );
    } catch {
  ...
  private validateBackupEnvelope(value: unknown): BackupEnvelope {
    ...
    try {
      ...
      throw new SystemAdministrationError(
        400,
        "invalid_backup",
        "The configuration backup is invalid or unsupported",
      );
    }

    assertUnique(
      envelope.data.deviceCategories.map((row) => row.id),
      "category IDs",
    );
    assertUnique(
      envelope.data.deviceCategories.map((row) => row.name),
      "category names",
      (value) => value.toLowerCase(),
    );
    assertUnique(
      envelope.data.deviceGroups.map((row) => row.id),
      "group IDs",
    );
    assertUnique(
      envelope.data.deviceGroups.map((row) => row.name),
      "group names",
      (value) => value.toLowerCase(),
    );
    assertUnique(
      envelope.data.devices.map((row) => row.id),
      "device IDs",
    );
    assertUnique(
      envelope.data.registers.map((row) => row.id),
      "register IDs",
    );
    assertUnique(
      envelope.data.alarmRules.map((row) => row.id),
      "alarm rule IDs",
    );
    ...
    assertUnique(
      envelope.data.dataServerDeviceExports.map((row) => row.device_id),
      "data-server device mappings",
    );
    for (const mapping of envelope.data.dataServerDeviceExports) {
      if (!deviceIds.has(mapping.device_id)) {
        throw new SystemAdministrationError(
          400,
          "invalid_backup",
          "A data-server mapping references an unknown device",
        );
      }
    }
    assertUnique(
      envelope.data.dataServerDeviceExports
        ...
        .map((row) => String(row.modbus_unit_id)),
      "enabled Modbus server unit IDs",
    );
    const dataServerSettings = envelope.data.dataServerSettings[0];
    if (
      dataServerSettings?.modbus_enabled === 1 &&
      !envelope.data.dataServerDeviceExports.some(
        (mapping) => mapping.modbus_enabled === 1,
      )
    ) {
      throw new SystemAdministrationError(
        400,
        "invalid_backup",
        "Enabled Modbus server settings require an exported device mapping",
      );
    }
    if (
      dataServerSettings?.opcua_enabled === 1 &&
      !envelope.data.dataServerDeviceExports.some(
        (mapping) => mapping.opcua_enabled === 1,
      )
    ) {
      throw new SystemAdministrationError(
        400,
        "invalid_backup",
        "Enabled OPC UA server settings require a published device",
      );
    }
    const historianTableOwners = new Map<string, string>();
    for (const device of envelope.data.devices) {
      if (
        (device.category_id && !categoryIds.has(device.category_id)) ||
        (device.group_id && !groupIds.has(device.group_id))
      ) {
        throw new SystemAdministrationError(
          400,
          "invalid_backup",
          "A device classification reference is invalid",
        );
      }
      if (
        device.postgres_enabled === 1 ||
        ...
        for (const tableName of [
          ...
          if (owner && owner !== device.id) {
            throw new SystemAdministrationError(
              400,
              "invalid_backup",
              "Historian table names must be unique across configured devices",
            );
          }
    ...
    for (const register of envelope.data.registers) {
      if (!deviceIds.has(register.device_id)) {
        throw new SystemAdministrationError(
          400,
          "invalid_backup",
          "A register device reference is invalid",
        );
      }
    }
    for (const rule of envelope.data.alarmRules) {
      if (!registerIds.has(rule.register_id)) {
        throw new SystemAdministrationError(
          400,
          "invalid_backup",
          "An alarm rule register reference is invalid",
        );
      }
    }
    assertUnique(
      envelope.data.registers.map((row) => `${row.device_id}\0${row.name}`),
      "register names for a device",
    );
    assertUnique(
      envelope.data.registers.flatMap((row) => [
      ...
      if (settings.password_encrypted) {
        try {
          ...
          throw new SystemAdministrationError(
            400,
            "invalid_backup",
            "The saved PostgreSQL secret cannot be decrypted with this key",
          );
        }
      }
    }
      ...
      if (settings.access_token_encrypted) {
        try {
          ...
          throw new SystemAdministrationError(
            400,
            "invalid_backup",
            "The saved WhatsApp access token cannot be decrypted with this key",
          );
        }
      }
    }
    ...
    const profile = encodedProfile
      ...
      : null;
    if (
      (profile === null) !== (envelope.data.openVpn.profile_name === null) ||
      ...
      throw new SystemAdministrationError(
        400,
        "invalid_backup",
        "The OpenVPN backup state is inconsistent",
      );
    }
    if (envelope.data.openVpn.profile_name) {
      safeUploadFilename(envelope.data.openVpn.profile_name, /\.ovpn$/i);
    }
    if (profile) validateOpenVpnProfile(profile);
    return envelope;
  }

  private parseBackup(backup: string): BackupEnvelope {
    if (
    ...
    try {
      ...
      return this.validateBackupEnvelope(JSON.parse(decrypted) as unknown);
    } catch (error) {
      if (error instanceof SystemAdministrationError) throw error;
      throw new SystemAdministrationError(
        400,
        "invalid_backup",
        "The configuration backup cannot be decrypted or parsed",
      );
    }
  }

  private assertBackupRecordLimits(): void {
    for (const [table, maximum] of Object.entries(BACKUP_RECORD_LIMITS)) {
      ...
      if (row.count > maximum) {
        throw new SystemAdministrationError(
          413,
          "configuration_backup_too_large",
          `The ${table} table exceeds the configuration backup record limit`,
        );
      }
    }
  }
  ...
  private selectAllIfExists<T>(table: string): T[] {
    return this.tableExists(table) ? this.selectAll<T>(table) : [];
  }
  ...
  private deleteOperationalConfiguration(): void {
    if (this.tableExists("data_server_device_exports")) {
      this.database.connection
  ...
  private resetDataServerConfiguration(updatedAt: string): void {
    if (!this.tableExists("data_server_settings")) return;
    if (this.tableExists("data_server_device_exports")) {
      this.database.connection
        .prepare("DELETE FROM data_server_device_exports")
        .run();
    ...
    this.database.connection
      .prepare(
        `INSERT INTO data_server_settings (
      ...
      .run(
        ...
        settings.modbus_bind_address,
        settings.modbus_port,
        settings.modbus_refresh_interval_ms,
        settings.opcua_enabled,
        settings.opcua_bind_address,
        settings.opcua_advertised_host,
        settings.opcua_port,
        settings.opcua_endpoint_path,
        settings.opcua_allow_anonymous,
        settings.opcua_refresh_interval_ms,
        settings.updated_at,
      );
  }

  private upsertUpdateState(state: UpdateStateRow): void {
    this.database.connection
      .prepare(
        `INSERT INTO system_update_state (
          id, staged_version, staged_filename, staged_sha256, staged_at,
          last_error
        ) VALUES (1, ?, ?, ?, ?, ?)
        ON CONFLICT(id) DO UPDATE SET
  ...
  private upsertOpenVpnState(state: OpenVpnStateRow): void {
    this.database.connection
      .prepare(
        `INSERT INTO openvpn_state (
          id, profile_name, enabled, last_changed_at, last_error
        ) VALUES (1, ?, ?, ?, ?)
        ON CONFLICT(id) DO UPDATE SET
  ...
  private recordUpdateError(message: string | null): void {
    this.database.connection
      .prepare("UPDATE system_update_state SET last_error = ? WHERE id = 1")
      .run(message);
  }

  private recordOpenVpnError(message: string | null): void {
    this.database.connection
    ...
    if (!this.managedFileExists(filePath)) {
      if (required) {
        throw new SystemAdministrationError(
          ...
          "A required managed system file is missing",
        );
      }
      return null;
    }
    ...
    try {
      ...
      if (metadata.size > maximumBytes) {
        throw new SystemAdministrationError(
          ...
          "A managed system file exceeds its allowed size",
        );
      }
  ...
  private recoverInterruptedManagedFiles(target: string): void {
    ...
    for (const filePath of temporary) rmSync(filePath, { force: true });
    ...
    for (const filePath of previous) rmSync(filePath, { force: true });
    if (recoveredFiles) fsyncDirectory(directory);
  }

  private replaceManagedFiles(
    replacements: ManagedFileReplacement[],
    databaseMutation: () => void,
  ): void {
    ...
    try {
      ...
      for (const item of prepared) {
        if (committed) rmSync(item.previous, { force: true });
        if (item.temporary) rmSync(item.temporary, { force: true });
        fsyncDirectory(path.dirname(item.target));
      }
    }
  }
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      child.once("exit", (code, signal) => {
        ...
        if (code === 0) {
          ...
          reject(
            new Error(
              `System helper exited with ${code ?? `signal ${signal ?? "unknown"}`}`,
            ),
          );
        }
      });
