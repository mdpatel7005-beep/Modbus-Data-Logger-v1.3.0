...
import {
  ...
  enabledTags,
  errorMessage,
  validateNetworkPublisherConfig,
} from "./protocol-publication.js";
...
export type OpcUaRuntimeFactory = (
  config: OpcUaPublisherConfig,
  devices: PublishedDevice[],
  callbacks: OpcUaRuntimeCallbacks,
...
function selectedDevices(
  ...
  if (publishedDeviceIds.length === 0) return [];
...
function validateOpcUaConfig(config: OpcUaPublisherConfig): void {
  ...
  if (
    ...
    throw new Error(
      "OPC UA advertised host must be a hostname or IP address without a URL scheme or path",
    );
  }
  if (
    ...
    throw new Error(
      "OPC UA advertised host is required when binding to all interfaces",
    );
  }
  if (
    !config.endpointPath.startsWith("/") ||
    config.endpointPath.includes("?") ||
    config.endpointPath.includes("#")
  ) {
    throw new Error(
      "OPC UA endpoint path must start with / and cannot contain ? or #",
    );
  }
...
function readingTimestamp(reading: PublishedReading | null): Date | null {
  if (!reading?.timestamp) return null;
  const timestamp = new Date(reading.timestamp);
  return Number.isNaN(timestamp.valueOf()) ? null : timestamp;
}

function countPublishedTags(devices: PublishedDevice[]): number {
  return devices.reduce(
    (count, device) => count + enabledTags(device).length,
    0,
  );
}
...
function certificateCoversHost(
  server: CertificateManagedOpcUaServer,
  host: string,
): boolean {
...
function invalidateCertificateCaches(
  server: CertificateManagedOpcUaServer,
): void {
 ...
 */
export async function ensureOpcUaCertificateIdentity(
  server: CertificateManagedOpcUaServer,
  ...
  if (
    server.certificateFile === "<in-memory>" ||
    server.certificateFile === "<unknown>"
  ) {
    throw new Error(
      `OPC UA certificate does not cover ${advertisedHost}; provision a replacement certificate before enabling this endpoint`,
    );
  }
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `regenerated certificate still lacks ${[
    ...
    if (/not self-signed/i.test(message)) {
      throw new Error(
        `OPC UA CA/GDS certificate does not cover ${advertisedHost}; provision a replacement certificate with the advertised host in its SAN`,
      );
    }
    throw new Error(
      `OPC UA self-signed certificate rotation failed for ${advertisedHost}; the previous certificate was restored: ${message}`,
    );
  }
}

function topologySignature(devices: PublishedDevice[]): string {
...
export async function createOpcUaServerRuntime(
  config: OpcUaPublisherConfig,
  devices: PublishedDevice[],
  callbacks: OpcUaRuntimeCallbacks,
): Promise<OpcUaServerRuntime> {
  ...
  const server = new OPCUAServer({
    ...
    alternateHostname: [
      ...new Set(
        [config.advertisedHost?.trim() || config.host, config.host].filter(
          ...
          userManager: {
            isValidUserAsync: (
              ...
              callbacks
                ...
                .then(
                  (isAuthorized) => callback(null, isAuthorized),
                  (error: unknown) =>
                    callback(
                      error instanceof Error
                        ? error
  ...
  const statusForReading = (reading: PublishedReading | null) => {
  ...
  try {
    ...
    const devicesFolder = namespace.addFolder(
    ...
    for (const device of devices) {
      ...
      for (const tag of enabledTags(device)) {
        ...
        const dataType =
          tag.dataType === "bool" ? DataType.Boolean : DataType.Double;
          ...
          value: {
            get: () =>
              new Variant({
                dataType: DataType.DateTime,
                value:
                  readingTimestamp(callbacks.getReading(tag.id)) ??
                  new Date(0),
              }),
          },
        });
    ...
    server.on("openSecureChannelFailure", () => {
      callbacks.onRuntimeError(
        new Error("OPC UA secure channel negotiation failed"),
      );
    });
    await server.start();
  } catch (error) {
    try {
      if (server.initialized) await server.shutdown(100);
    } catch {
  ...
  return {
    ...
    close: async (timeoutMs) => {
      ...
      const timedOut = new Promise<never>((_resolve, reject) => {
        timeout = setTimeout(
          () => reject(new Error("OPC UA server shutdown timed out")),
          timeoutMs,
        );
      });
      try {
...
export class OpcUaPublisher {
  ...
  private config: OpcUaPublisherConfig;
  private runtime: OpcUaServerRuntime | null = null;
  private readings = new Map<string, PublishedReading>();
  private publishedTopology = "";
  private refreshTimer: NodeJS.Timeout | null = null;
  private status: OpcUaPublisherStatus;

  constructor(
    private readonly source: ProtocolPublicationSource,
    config: OpcUaPublisherConfig,
    private readonly runtimeFactory: OpcUaRuntimeFactory =
      createOpcUaServerRuntime,
    private readonly authenticationProvider?: OpcUaAuthenticationProvider,
  ) {
  ...
  getStatus(): OpcUaPublisherStatus {
    ...
    return {
      ...
      currentSubscriptions:
        runtime?.getCurrentSubscriptionCount() ?? 0,
  ...
  start(): Promise<void> {
    return this.lifecycle.run(async () => {
      if (this.runtime || this.status.state === "starting") return;
  ...
  async refresh(): Promise<void> {
    try {
      const devices = selectedDevices(
        await this.source.getPublishedDevices(),
        this.config.publishedDeviceIds,
      );
      const readings = new Map<string, PublishedReading>();
      for (const device of devices) {
        for (const tag of enabledTags(device)) {
          if (tag.reading) readings.set(tag.id, tag.reading);
        }
      }
      ...
      if (
        ...
        await this.lifecycle.run(async () => {
          if (!this.runtime || this.status.state !== "running") return;
          await this.stopUnsafe(false);
          await this.startUnsafe();
        });
      }
  ...
  private async startUnsafe(): Promise<void> {
    ...
    try {
      ...
      if (!this.config.allowAnonymous && !this.authenticationProvider) {
        throw new Error(
          "OPC UA anonymous access is disabled but no authentication provider is configured",
        );
      }
      const devices = selectedDevices(
        await this.source.getPublishedDevices(),
      ...
      this.readings = new Map(
        devices.flatMap((device) =>
          enabledTags(device)
            ...
            .map((tag) => [tag.id, tag.reading!] as const),
        ),
      );
      this.status.publishedDevices = devices.length;
      this.status.publishedTags = countPublishedTags(devices);
        ...
        ...(this.authenticationProvider
          ? {
              authenticateUser: (username: string, password: string) =>
                this.authenticationProvider!.validateCredentials(
                  username,
                  password,
                ),
            }
  ...
  private async stopUnsafe(explicitStop: boolean): Promise<void> {
